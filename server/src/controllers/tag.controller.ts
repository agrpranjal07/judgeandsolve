...
export const createTag = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    return successResponse(res, { statusCode: 201, message: 'Tag created', data: tag });
  } catch (err) {
    next(err);
  }
};
...
export const getTags = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    return successResponse(res, { statusCode: 200, message: 'Tags fetched', data: tags });
  } catch (err) {
    next(err);
  }
};
...
export const setTags = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!Array.isArray(tagIds) || tagIds.length === 0) {
      throw new ApiError(400, 'tagIds array is required');
    }
...
export const removeTag = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!deleted) throw new ApiError(404, 'Tag association not found');
